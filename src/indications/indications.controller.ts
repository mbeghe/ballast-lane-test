...
export class IndicationsController {
  ...
  @Post()
  @ApiResponse({ status: 201, description: 'Indication created', type: IndicationWithProgramResponseDto })
  async create(@Body() dto: CreateIndicationDto): Promise<IndicationWithProgramResponseDto> {
  ...
  @Patch(':id')
  @ApiResponse({ status: 200, description: 'Indication updated', type: IndicationWithProgramResponseDto })
  ...
  @Get(':id')
  @ApiResponse({ status: 200, description: 'Get indication by ID', type: IndicationWithProgramResponseDto })
  @ApiResponse({ status: 404, description: 'Indication not found' })
  async findOne(@Param('id', ParseIntPipe) id: number): Promise<IndicationWithProgramResponseDto> {
    return this.indicationsService.findOne(id);
  }

  @Get()
  @ApiResponse({ status: 200, description: 'List of all indications', type: IndicationWithProgramResponseDto, isArray: true })
